...
export class CreditsComponent implements OnInit {
  ...
  async openAdd() {
    if (DateUtils.isClosed() && !DateUtils.hasChoiceBeenMade()) {
      const result = await Swal.fire({
        title: 'الصندوق مسدود!',
        text: 'الصندوق ديال اليوم تسد. واش بغيتي تقيد هاد الدين فاليوم لي فات ولا فاليوم الجديد؟',
        icon: 'warning',
  ...
  parseDescription(desc: string): { header: string; items: string[] } {
    ...
    // Legacy format (" و " separator generated by old code)
    if (desc.startsWith('باقي من ثمن') && desc.includes(' و ')) {
      const header = 'باقي من ثمن';
      const itemsStr = desc.substring(header.length).trim();
      const items = itemsStr.split(' و ').map(s => s.trim()).filter(s => s.length > 0);
      return { header, items };
    }

    // Default: no separator found, render as plain text
